# -*-  encoding:utf-8  -*-
def get_result_five_fold_cross(model,normal_file,recurrence_file,surgery_file,image,cnn,train_caffemodel):
	import numpy as np
	...
	MODEL_FILE = caffe_root+'myself/'+cnn+'/'+model+'/deploy-101.prototxt'
	PRETRAINED = caffe_root+'myself/'+cnn+'/'+model+'/'+'caffe_alexnet_train_'+train_caffemodel+'_iter_2000.caffemodel'+''

	#TEST_FILE = '../data/liris-accede/test_arousal.txt'

	MEAN_FILE = caffe_root+'myself/'+cnn+'/'+model+'/mnist_train_lmdb_mean.binaryproto'
	...
	RESULT_FILE=caffe_root+'myself/'+cnn+'/weighted-fine-resnet-128-10/result_cnn-10-128/'+'caffe_alexnet_train_'+train_caffemodel+'.txt'
