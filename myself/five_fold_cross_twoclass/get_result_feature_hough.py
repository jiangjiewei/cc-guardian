# -*-  encoding:utf-8  -*-
def get_result_feature_hough(model,ok_file,other_file,cnn,train_caffemodel):
	import numpy as np
	...
	test_other='/home/jie/桌面/cnn_hough/'+other_file+'/test'
	test_ok='/home/jie/桌面/cnn_hough/'+ok_file+'/test'
	feature_other='/home/jie/桌面/cnn_hough/result_cnn/'+cnn+'/'+model+'_otherfeature.txt'
	feature_ok='/home/jie/桌面/cnn_hough/result_cnn/'+cnn+'/'+model+'_okfeature.txt'

	sum_other=0
	error_other_number=0
	list_other=[]
	otherp=['D']
	for root,dirs,files in os.walk(test_other,topdown=False):
		#file1=sorted(files)
		list.sort(files)
		for file in files:
			print file
